...
const ProductItem = ({ filter }) => {
    ...
    useEffect(() => {
        const debounceFilter = setTimeout(() => {
            ...
            if (filter.keyword) {
                tempFilteredProduct = tempFilteredProduct?.filter((item) => item.Medname.toLowerCase().includes(filter.keyword.toLowerCase()));
            }
            if (selectedCategory) {
                            ...
                            <div className="space-y-3 " key={Product?._id}>
                                <div className="flex items-center justify-center rounded-md border border-[#0360D9]/30 bg-white p-4">
                                    <Link to={`/detailsMed/${Product._id}`}>
                                        <img className="max-w-[144px] h-40" src={Product?.Image} alt={Product?.Medname} />
                                    </Link>
                                </div>
                                <div className="space-y-3">
                                    <div className="space-y-3 pl-2">
                                        <h4 className="text-lg font-bold lg:text-xl">{Product?.Medname}</h4>
                                        ...
                                        <Link to={`/company/${Product?.Company}`}>
