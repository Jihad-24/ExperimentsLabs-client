...
const CartProduct = ({ Product }) => {
    const { Medname, Image, Price, Category, Company, _id } = Product.Product;
    const Axious = UseAxiosPublic();

    const [cartdata, refetchCart] = UseCart();

    const handleRemove = () => {
        Axious.delete(`/CartProduct/${Product?.OrderId}`).then((res) => {
    ...
    const UpdateQuantity = (way, id) => {
        ...
        Axious.put(`/updateQuantity/${id}`, { quantity: final }).then((res) => {
            console.log(res.data);
            refetchCart();
        });
    };
    ...
    return (
        <li className="flex flex-col md:flex-row justify-between items-start gap-4 py-4 border-b border-gray-300">
            <div className="flex flex-row items-center gap-8 sm:min-w-[500px] space-y-4 md:space-y-0 md:space-x-4">
                <img alt="T-shirt" className="w-16 h-16 bg-gray-200 rounded object-cover" src={Image} />
                <div>
                    <h3 className="font-semibold text-sm sm:text-base md:text-lg lg:text-xl">{Medname}</h3>
                ...
                <div className="flex gap-2 items-center  md:text-xl sm:text-lg text-sm ">
                    {Product?.quantity === 1 ? (
                        ...
                        <>
                            <div
                                onClick={() => {
                                    UpdateQuantity('minus', Product?.OrderId);
                                }}
                    ...
                    <div
                        onClick={() => {
                            UpdateQuantity('add', Product?.OrderId);
                        }}
